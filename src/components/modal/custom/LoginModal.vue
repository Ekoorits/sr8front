    ...
    <template #body>
      ...
      <div class="form-floating mb-3">
        <p style="text-align: left">Kui sul pole veel kasutajat
          <button @click="$emit('open-new-user-modal');$emit('event-close-modal')" type="submit" class="btn btn-outline-success ms-2">Loo uus konto</button>
        </p>
      </div>
    </template>
    <template #buttons>
      <button @click="$emit('event-close-modal')" type="submit" class="btn btn-outline-secondary me-3">Sulge</button>
      <button @click="$emit('event-log-out-executed')" type="submit" class="btn btn-outline-success">Jah</button>
    </template>


  </Modal>
  <NewUserModal :login-modal-is-open="newUserModalIsOpen"
